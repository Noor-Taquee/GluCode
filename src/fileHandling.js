function createFileButton(fileId, btnIcon, btnText, clickFunction) {
  ...
  const div1 = createElement("div", {
    className: "file-btn-info-div"
  });
  div1.appendChild(btnIcon);
  div1.appendChild(createElement("input", {
    type: "text",
    value: btnText,
    readOnly: true
  }));
  ...
  btn.appendChild(createElement("button", {
    className: "option-btn",
  }, [createIcon("bold", "dots-three-vertical")], {"click": () => {
    fileInFocus = fileId;
    showOptionsPanel();
  }
}));
...
const newFileForm = createElement("div", { className: "added-panel new-file-form" });
innerApp.appendChild(newFileForm);
newFileForm.addEventListener("animationend", () => {
  newFileForm.style.animation = "none";
})

newFileForm.appendChild(createElement("p", { className: "added-panel-header", textContent: "Enter file name" }));
...
const newFileFormBtnDiv = createElement("div", { className: "added-panel-btn-div" });
newFileForm.appendChild(newFileFormBtnDiv);

newFileFormBtnDiv.appendChild(createElement("button", { className: "added-panel-btn", textContent: "Cancel", onClick: () => {
  newFileNameInput.value = "";
  hideNewFilePanel();
} }));

newFileFormBtnDiv.appendChild(createElement("button", { className: "added-panel-btn", textContent: "Add File", onClick: () => {
...
async function loadFiles() {
  for (let fileId of Object.keys(allFiles)) {
    let fileBtn = createFileButton(fileId, createElement("i", { className: `bi bi-${fileIconNameMap[allFiles[fileId].extension].icon}` }), `${allFiles[fileId].name}.${allFiles[fileId].extension}`, () => { openFile(fileId) });
    fileCount++;
  }
}
...
const optionsPanel = createElement("div", { className: "added-panel options-panel" });
...
optionsPanel.appendChild(createElement("button", {
  id: "rename-btn",
  className: "tab-btn",
  onclick: showRenamePanel,
}, [createElement("i", { className: `ph-regular ph-pencil` }), createElement("span", { textContent: "rename" })]));

optionsPanel.appendChild(createElement("button", {
  id: "delete-btn",
  className: "tab-btn",
  onclick: showDeletePanel,
}, [createElement("i", { className: `ph-regular ph-trash` }), createElement("span", { textContent: "delete" })]));

optionsPanel.appendChild(createElement("button", {
  id: "cancel-btn",
  className: "tab-btn",
  onclick: hideOptionsPanel,
}, [createElement("i", { className: `ph-regular ph-x` }), createElement("span", { textContent: "cancel" })]));

//#region rename panel
const renamePanel = createElement("div", { className: "added-panel rename-panel" });
innerApp.appendChild(renamePanel);
renamePanel.addEventListener("animationend", () => {
  renamePanel.style.animation = "none";
})

renamePanel.appendChild(createElement("p", { className: "added-panel-header", textContent: "Rename file" }));
...
const renamePanelBtnDiv = createElement("div", { className: "added-panel-btn-div" });
renamePanel.appendChild(renamePanelBtnDiv);

renamePanelBtnDiv.appendChild(createElement("button", {
  id: "cancel-btn",
  className: "added-panel-btn",
  onclick: hideRenamePanel,
}, [createElement("i", { className: `ph-regular ph-x` }), createElement("span", { textContent: "cancel" })]));
...
//#region delete panel
const deletePanel = createElement("div", { className: "added-panel delete-panel" });
innerApp.appendChild(deletePanel);
deletePanel.addEventListener("animationend", () => {
  deletePanel.style.animation = "none";
})

deletePanel.appendChild(createElement("p", {
  className: "added-panel-header",
  textContent: "Delete file"
}));

deletePanel.appendChild(createElement("p", { className: "added-panel-text delete" }));
...
const deletePanelBtnDiv = createElement("div", { className: "added-panel-btn-div" });
deletePanel.appendChild(deletePanelBtnDiv);

deletePanelBtnDiv.appendChild(createElement("button", {
  id: "cancel-btn",
  className: "added-panel-btn",
...
deletePanelBtnDiv.appendChild(createElement("button", {
  id: "delete-btn",
  className: "added-panel-btn",
  onclick: () => deleteFile(fileInFocus),
}, [createElement("i", { className: `ph-regular ph-trash` }), createElement("span", { textContent: "Delete" })]));
...
function loadMode(name) {
  return new Promise((resolve) => {
    ...
    const s = createElement("script", {
      id: id,
      src: `https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.16/mode/${name}/${name}.min.js`,
      onload: resolve,
    });
